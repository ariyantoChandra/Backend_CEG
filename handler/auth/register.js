...
export const register = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < teamsList.length; i++) {
        ...
        // A. Insert User
        const [existing] = await connection.execute("SELECT id FROM user WHERE nama_tim = ?", [team.nama_tim]);
        if (existing.length > 0) throw new Error(`NAMA_TIM_EXIST: ${team.nama_tim}`);
        ...
        if (typeof membersArray === 'string') {
            try { membersArray = JSON.parse(membersArray); } catch (e) { membersArray = []; }
        }
